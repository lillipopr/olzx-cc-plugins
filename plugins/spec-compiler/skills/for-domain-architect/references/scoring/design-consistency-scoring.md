...
| 维度 | 权重 | 评分要点 |
|------|------|---------|
| 章节间一致性 | 30 分 | 各章节设计的一致性、无冲突 |
| 设计完整性 | 25 分 | 设计覆盖所有必要内容 |
| 原则遵循度 | 25 分 | 对原则文件的遵循程度 |
...
**检查项**：
- [ ] 聚合设计完整（3分）：聚合根、实体、值对象
- [ ] 领域服务设计完整（2分）
- [ ] 领域事件设计完整（3分）：事件定义、发布、订阅
- [ ] 聚合图完整（2分）
...
**评分标准**：
- 优秀（90-100分）：完全遵循 domain-event.md 原则
- 良好（80-89分）：90%以上遵循
- 及格（60-79分）：70%以上遵循
- 不及格（<60分）：低于70%遵循

**检查项**：
- [ ] 领域事件特征原则（1分）：domain-event.md 1.1-1.2
- [ ] 领域事件设计原则（2分）：domain-event.md 4.1-4.3
...
| 事件关联聚合 | 3 | __ | 领域事件与聚合 |
| 在聚合根中发布 | 3 | __ | 领域事件与聚合 |
| 事件数据来自聚合 | 2 | __ | 领域事件与聚合 |
| **设计完整性** | | | |
| 业务能力树完整 | 2 | __ | 战略设计 |
| 上下文划分完整 | 2 | __ | 战略设计 |
| 上下文映射图完整 | 2 | __ | 战略设计 |
...
| 领域事件特征原则 | 1 | __ | domain-event.md |
| 领域事件设计原则 | 2 | __ | domain-event.md |
| **检查清单对应** | | | |
...
1. 章节间一致性：29/30 分
   ...
   - 聚合与领域服务：7/7 分（完全一致）
   - 领域服务与应用层：7/7 分（完全一致）
   - 领域事件与聚合：7/8 分（基本一致，扣1分因为有一个事件的携带数据说明不够清晰）
...
3. 原则遵循度：24/25 分
   ...
   - 领域服务原则：5/5 分（完全遵循）
   - 应用层原则：4/4 分（完全遵循）
   - 领域事件原则：2/3 分（基本遵循，扣1分因为有一个事件命名略不规范）
