...
从 《聚合设计》《领域服务设计》中提取以下信息：

| 来源 | 提取内容 | 转化目标 |
|------|---------|-----------|
| 聚合设计 | 聚合根行为 | 聚合根行为直接暴露 |
| 聚合设计 | 聚合根行为中的事件发布 | 事件监听器设计 |
| 领域服务 | 领域服务 | 应用服务协调 |
...
**目标**：设计事件处理器，处理聚合根发布的领域事件。

#### 4.5.1 事件监听器识别

| 监听处理方法名称 | 监听的事件 | 处理逻辑 | 影响的聚合 | 所属应用服务 |
|----------|-----------|---------|-----------|-------------|
| {监听处理方法1}   | {事件1} | {逻辑描述} | {聚合A} | {当前服务} |
| {监听处理方法2}   | {事件2} | {逻辑描述} | {聚合B} | {当前服务} |

**事件监听器示例**：

| 监听器名称 | 监听的事件 | 处理逻辑 | 影响的聚合 | 所属应用服务 |
|---------------------|-----------|---------|-----------|-------------|
| CouponGrantHandler  | MembershipActivatedEvent | 开始每日点券发放 | Coupon | CouponApplication |
| NotificationHandler | MembershipActivatedEvent | 发送激活通知 | - | NotificationApplication |
| AuditHandler        | MembershipActivatedEvent | 记录激活日志 | AuditLog | AuditApplication |
